...
- Default: `-` (disabled; the ACL is only set when explicitly configured to `1`)
- Example: `1`
...
When set to `1`, LobeHub will convert videos to base64 encoding before uploading them to the LLM model.
