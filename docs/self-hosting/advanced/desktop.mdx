...
<Callout>
  `OIDC_JWKS_KEY` has been renamed to `JWKS_KEY`. If your deployment still has `OIDC_JWKS_KEY` set,
  you need to manually rename it to `JWKS_KEY` — there is no automatic fallback.
</Callout>
