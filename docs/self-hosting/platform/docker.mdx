...
description: >-
  Learn how to deploy the LobeHub server database version using Docker on Linux and local machines.
tags:
  - LobeHub
  - Docker
  - Database Deployment
...
The above command will create a Postgres instance named `my-postgres` with the network set to `pg`, where `paradedb/paradedb:latest-pg17` is a Postgres 17 image with the `pgvector` and `pg_search` extensions installed and enabled by default.
  ...
  <Callout type="info">
    ...
    It should be noted that the new version of ParadeDB requires explicitly enabling the pg\_search extension in order to be used by LobeHub. Here we explicitly enable pg\_search.
