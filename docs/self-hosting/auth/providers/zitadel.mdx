...
description: >-
  Learn how to configure ZITADEL SSO for LobeHub, including creating an application and setting up environment variables.
tags:
  - ZITADEL
  - Authentication
  - LobeHub
...
<Callout type={'warning'}>
  **Deprecated Environment Variables**: `ZITADEL_CLIENT_ID` / `ZITADEL_CLIENT_SECRET` /
  `ZITADEL_ISSUER` are no longer supported. Setting any of them will cause the deployment to fail at
  build/startup — use `AUTH_ZITADEL_ID` / `AUTH_ZITADEL_SECRET` / `AUTH_ZITADEL_ISSUER` instead.
</Callout>
