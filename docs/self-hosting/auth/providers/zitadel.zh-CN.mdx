...
<Callout type={'warning'}>
  **已废弃的环境变量**：`ZITADEL_CLIENT_ID` / `ZITADEL_CLIENT_SECRET` / `ZITADEL_ISSUER`
  已不再支持，设置其中任意一个都会导致部署在构建 / 启动阶段失败，请改用 `AUTH_ZITADEL_ID` /
  `AUTH_ZITADEL_SECRET` / `AUTH_ZITADEL_ISSUER`。
</Callout>
