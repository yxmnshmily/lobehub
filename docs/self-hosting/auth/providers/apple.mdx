...
description: >-
  Learn how to configure Apple Sign In for LobeHub, including creating credentials in Apple Developer Portal and setting up environment variables.
tags:
  - Apple
  - Authentication
  - LobeHub
...
<Callout type={'warning'}>
  Apple Sign In requires a paid Apple Developer account ($99/year) and does not support localhost.
  You must use a domain with HTTPS for both development and production.
</Callout>
...
<Callout type={'warning'}>
  The JWT expires after maximum 180 days. You need to regenerate and update it before expiration.
</Callout>
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate with Apple and use LobeHub.
</Callout>
