...
description: >-
  Learn how to add a new bot platform (e.g., Slack, WhatsApp) to LobeHub's channel system, including schema definition, client implementation, and platform registration.
tags:
  - Bot Platform
  - Message Channels
  - Integration
...
| `sanitizeUserInput()` | _(Optional)_ Strip bot mention artifacts from user input    |
| `shouldSubscribe()`   | _(Optional)_ Control thread auto-subscription behavior      |
| `formatReply()`       | _(Optional)_ Append platform-specific formatting to replies |
