...
description: >-
  Explore the client-server interaction logic of LobeHub Chat API, including event sequences and core components.
tags:
  - Chat API
...
After the user sends a message, `sendMessage()` (`src/store/chat/slices/agentRun/actions/entries/conversationLifecycle.ts`) creates the user message and assistant message placeholder, then calls `executeClientAgent()`.
...
**Initialization** (`src/store/chat/slices/agentRun/actions/transports/client/streamingExecutor.ts`):
...
- A tRPC route `apps/server/src/routers/lambda/aiChat.ts` also exists for server-side message sending and structured output scenarios
...
- Frontend processes the streaming response via `fetchSSE` and [fetchEventSource](https://github.com/Azure/fetch-event-source)
...
**MCP Tools**: External tools connected via [Model Context Protocol](https://modelcontextprotocol.io/).

- Frontend calls `MCPService` (`src/services/mcp.ts`) via the `invokeMCPTypePlugin` method
- Supports three connection modes: stdio, HTTP (streamable-http/SSE), and cloud (Composio)
...
- **Client-side loop** (browser): Regular 1:1 chat, continue generation, group orchestration decisions. The loop runs in the browser, entry point is `executeClientAgent()` (`src/store/chat/slices/agentRun/actions/transports/client/streamingExecutor.ts`)
- **Server-side loop** (queue/local): Group chat supervisor agent, sub-agent tasks, API/Cron triggers. The loop runs on the server, streaming events to the client via SSE, entry point is `AgentRuntimeService.executeStep()` (`apps/server/src/services/agentRuntime/AgentRuntimeService.ts`), tRPC route at `apps/server/src/routers/lambda/aiAgent.ts`
