...
请参考 [开发环境设置指南](/zh/docs/development/basic/setup-development) 了解完整的环境搭建流程，包括软件安装、项目配置、Docker 服务启动和数据库迁移等步骤。
...
默认语言文件位于 `packages/locales/src/default/`（英文），翻译文件位于 `locales/` 目录。开发时只需编辑 `packages/locales/src/default/` 中的 key，CI 会自动生成其他语言的翻译文件。
