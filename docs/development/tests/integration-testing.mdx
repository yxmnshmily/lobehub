...
description: >-
  Learn how to effectively conduct integration testing to verify module interactions and data integrity.
tags:
  - Integration Testing
  - Software Testing
  - Test Automation
...
```bash
# Run all integration tests (server package)
...
├── task.integration.test.ts               # Task Router tests
└── aiAgent/                               # Agent Runtime integration tests
...
describe('Your Feature Integration Tests', () => {
  ...
  it('should do something', async () => {
    ...
    // 2. Perform operation
    const result = await caller.someMethod({/* params */});
...
it('should prevent cross-user access', async () => {
  ...
  const sessionId = await sessionRouter
    .createCaller(createTestContext(userA))
    .createSession({/* ... */});
