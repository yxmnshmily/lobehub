...
description: >-
  Learn how to create a Slack app and connect it to your LobeHub agent as a message channel, enabling your AI assistant to interact with users on Slack through channels and direct messages.
tags:
  - Slack
  - Message Channels
  - Bot Setup
...
A populated **Allowed User IDs** field is a global gate — DMs _and_ channel `@mentions` are restricted to listed Slack member IDs. Empty means "no user-level filter". Open a user's profile → click `⋮` → **Copy member ID** (starts with `U`).
