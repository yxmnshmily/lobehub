...
description: >-
  Learn how to create a Feishu custom app and connect it to your LobeHub agent as a message channel, enabling your AI assistant to interact with team members on Feishu.
tags:
  - Feishu
  - 飞书
  - Message Channels
...
A populated **Allowed User IDs** field is a global gate — DMs _and_ group `@mentions` are restricted to listed Feishu `open_id` values. Empty means "no user-level filter". Read the `open_id` from the event payload, or copy the **User ID** displayed in the Feishu Developer Portal.
