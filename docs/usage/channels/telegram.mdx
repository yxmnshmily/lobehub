...
description: >-
  Learn how to create a Telegram bot and connect it to your LobeHub agent as a message channel, enabling your AI assistant to chat with users on Telegram private and group conversations.
tags:
  - Telegram
  - Message Channels
  - Bot Setup
...
<Callout type={'warning'}>
  **About Group Privacy Mode:** Telegram bots have privacy mode enabled by default, which means they
  only receive messages that @mention the bot, reply to the bot, or contain /commands. If you change
  the privacy mode setting after creating the bot, you **must remove and re-add the bot to the
  group** for the new setting to take effect in that group.
</Callout>
...
A populated **Allowed User IDs** field acts as a global gate — DMs _and_ group `@mentions` are restricted to listed Telegram numeric user IDs. Empty means "no user-level filter". Grab a user's numeric ID via [@userinfobot](https://t.me/userinfobot), or read the `from.id` field from the bot's incoming update.
