...
title: Messenger 概览
description: >-
  将 LobeHub 账号一键关联到 Telegram、Slack、Discord 上的官方 LobeHub 机器人。只需绑定一次，选择默认 Agent，就能在常用聊天工具里直接和你的 AI 助手对话，无需自建机器人。
tags:
  - Messenger
  - Telegram
  - Slack
...
| 操作 | 效果 |
| --- | --- |
| 在 _user_ 行点 **断开** | 解除 **你自己** 的 LobeHub 账号与该平台账号的关联。机器人不再接收你的消息，直到你重新 `/start`。 |
| 在 _workspace / server_ 行点 **断开** | 移除安装审计记录。Slack 会因此让该工作区的整个机器人失效；Discord 上机器人会继续留在服务器，直到 Discord 管理员把它踢出。 |
