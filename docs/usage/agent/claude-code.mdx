...
title: Claude Code
description: >-
  Delegate Anthropic's Claude Code inside LobeHub — chat with the Claude Code CLI from your desktop app, watch tasks, todos, skills, and tool calls stream in real time, and resume sessions across turns.
tags:
  - LobeHub
  - Claude Code
  - Coding Agent
...
If you change folders and the saved Claude Code session can't be resumed, LobeHub shows: _"Working directory changed. Previous Claude Code session can only be resumed from its original directory, so a new conversation has started."_
