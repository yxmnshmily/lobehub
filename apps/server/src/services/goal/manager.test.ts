...
describe('CLI main Agent planning', () => {
  it('validates and persists group assignments atomically through durable manager submission', async () => {
    await db.insert(agents).values({ id: 'manager-member', userId });
    await db.insert(chatGroups).values({ id: 'manager-group', userId });
    await db.insert(chatGroupsAgents).values([
      { chatGroupId: 'manager-group', agentId, userId, role: 'supervisor' },
      { chatGroupId: 'manager-group', agentId: 'manager-member', userId, role: 'participant' },
    ]);
    const graph = await service().create({
      title: 'Group research',
      createdByAgentId: agentId,
      config: { groupId: 'manager-group', manager: { maxTurns: 4 } },
    });
    ...
    await expect(
      manager().submit(graph.goal.id, state.token, op!.id, taskPlan),
    ...
    await manager().submit(graph.goal.id, state.token, op!.id, {
      ...taskPlan,
      tasks: [{ ...taskPlan.tasks[0], assigneeAgentId: 'manager-member' }],
    });
    const after = await service().graph(graph.goal.id);
    const node = after.nodes.find((node) => node.kind === 'task')!;
    expect(after.goal.config?.taskAssignments?.[node.id]).toBe('manager-member');
    expect(after.goal.config?.managerState?.submitted?.action).toBe('tasks');
  });
