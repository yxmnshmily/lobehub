...
export class GoalService {
  ...
  private planDecomposition = async (graph: GoalGraphSnapshot, effects: GoalAdvanceEffect[]) => {
    ...
    try {
      ...
      let planningFailure:
        { goalId: string; message: string; nodeId?: string; outcome: 'waiting_human' } | undefined;
      ...
      const committed = await this.db.transaction(async (tx) => {
        ...
        if (fresh.goal.config?.groupId !== graph.goal.config?.groupId) return undefined;
        const currentGroupExecution = await resolveGroupExecution(
          tx,
          this.userId,
          this.workspaceId,
          fresh.goal.config,
        ...
        const invalidAssignments =
          ...
          plan?.tasks.some(
            (draft) =>
              ...
              !currentGroupExecution!.members.some(
                (member) => member.agentId === draft.assigneeAgentId,
              ),
          );
        if (groupExecution && (!plan?.tasks.length || invalidAssignments)) {
          ...
          if (node) {
            if (problem) await writer.createEdge(goalId, problem.id, node.id, 'leads_to');
            ...
            committedEffects.push({ type: 'opened_decision', nodeId: node.id, detail: reason });
          }
          // claimPlanning already moved the persisted row to running.
          await goalModel.update(goalId, { status: 'review' });
          await writer.recordGoalStatus(goalId, 'running', 'review', reason);
          committedEffects.push({ type: 'goal_status', detail: 'review' });
          planningFailure = { goalId, message: reason, nodeId: node?.id, outcome: 'waiting_human' };
        ...
        const taskAssignments = { ...fresh.goal.config?.taskAssignments };
        ...
        for (const draft of draftTasks) {
          ...
          if (currentGroupExecution && draft.assigneeAgentId)
        ...
        if (currentGroupExecution)
          await goalModel.update(goalId, { config: { ...fresh.goal.config, taskAssignments } });
