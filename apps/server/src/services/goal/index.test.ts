...
describe('GoalService', () => {
  ...
  it('starts the bound Task once when advances race on dispatch', async () => {
    ...
    const runSpy = vi
      .spyOn(TaskRunnerService.prototype, 'runTask')
      .mockImplementation(async function ({ taskId }) {
        return { taskId } as never;
      });
    const service = new GoalService(serverDB, userId);
  ...
  it('plans the decomposition once when two advances race through the planner', async () => {
    ...
    let plannerCalls = 0;
    vi.spyOn(GoalCriteriaGeneratorService.prototype, 'decompose').mockImplementation(
      async function () {
        plannerCalls += 1;
        await gate;
        return {
    ...
    async (change) => {
      ...
      vi.spyOn(GoalCriteriaGeneratorService.prototype, 'decompose').mockImplementation(
        async function () {
          ...
          return {
            problemStatement: 'Old plan',
            tasks: [{ instruction: 'Old', title: 'Old task' }],
          };
        },
      );
      await service.tick(graph.goal.id);
