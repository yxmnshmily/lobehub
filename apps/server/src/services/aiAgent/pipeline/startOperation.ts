...
export interface StartOperationInput {
  ...
  maxSteps?: number;
  /** Release server-owned billing resources when operation startup fails. */
  onOperationStartFailed?: () => Promise<void>;
  operationId: string;
  operationTaskId?: string;
  parentOperationId?: string;
  /** Trusted, in-process principal context for platform-managed execution. */
  ...
  try {
    const result = await deps.agentRuntimeService.createOperation({
      ...
      appContext: {
        ...
        ...(platformManagedExecution
          ? {
              billingActorUserId: platformManagedExecution.actorUserId,
              ...(platformManagedExecution.hostedGroupRun
                ? { hostedGroupRun: platformManagedExecution.hostedGroupRun }
                : {}),
              resourceOwnerUserId: platformManagedExecution.resourceOwnerUserId,
            }
          : {}),
    ...
    await onOperationStartFailed?.().catch(() => undefined);
