...
export class AiAgentService {
  ...
  constructor(
    ...
    this.messageModel = new MessageModel(db, this.userId, wsId, undefined, messageModelOptions);
    ...
    this.topicModel = new TopicModel(db, this.userId, wsId, undefined, topicModelOptions);
  ...
  private get subAgentRunDeps(): SubAgentRunDeps {
    return {
      agentOperationModel: this.agentOperationModel,
      agentRuntimeService: this.agentRuntimeService,
      execAgent: (p) => this.execAgentWithInheritedPlatformBudget(p),
      messageModel: this.messageModel,
      threadModel: this.threadModel,
      userId: this.userId,
    };
  ...
  private async execAgentWithInheritedPlatformBudget(
    params: InternalExecAgentParams,
  ): Promise<ExecAgentResult> {
    const sharedBudget = params.parentOperationId
      ? getPlatformUsageSharedBudgetForOperation(params.parentOperationId, {
          actorUserId: this.actorUserId,
          workspaceId: this.workspaceId,
        })
      : undefined;
    return this.execAgent(
      sharedBudget ? grantPlatformManagedExecution({ ...params }, { sharedBudget }) : params,
    );
  }
   ...
   * direct users must be active global super administrators.
   */
    ...
    return startOperation(
      ...
      runContext,
      {
        ...
        onOperationStartFailed: () =>
          completePlatformUsageSharedBudgetForOperation(operationId).then(() => undefined),
        parentOperationId,
        platformManagedExecution: platformManagedExecutionAuthorized
          ? {
              actorUserId: this.actorUserId,
              hostedGroupRun: (appContext as typeof appContext & { hostedGroupRun?: unknown })
                ?.hostedGroupRun,
