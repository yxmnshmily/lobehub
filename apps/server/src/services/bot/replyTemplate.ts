...
const SYSTEM_STRINGS: Partial<Record<BotReplyLocale, SystemStrings>> = {
  'en-US': {
    ...
    cmdFeedbackSubmitted: 'Thanks — your feedback has been sent to the 旅游群 team.',
    cmdFeedbackSubmittedWithLink: (issueUrl) =>
      `Thanks — your feedback has been sent to the 旅游群 team. Tracked at: ${issueUrl}`,
    cmdFeedbackUsage:
      'Usage: `/feedback <your message>` — sends feedback directly to the 旅游群 team (no AI reply).',
    ...
    errorInsufficientCredits:
      "**Not enough credits.**\nThe credits this bot runs on can't cover this model's estimated cost. Ask the bot owner to top up credits or upgrade their plan on the 旅游群 website, or switch to a less expensive model in the agent's settings.",
    ...
    errorSubscriptionPlanLimit:
      "**Plan limit reached.**\nThe subscription plan this bot runs on can't cover this request — either its credits are used up for this billing period, or the plan doesn't include this model. Ask the bot owner to upgrade or top up their plan on the 旅游群 website, or configure a custom model API in the agent's provider settings.",
  ...
  'zh-CN': {
    ...
    cmdFeedbackSubmitted: '已收到，感谢反馈，已转交 旅游群 团队。',
    cmdFeedbackSubmittedWithLink: (issueUrl) =>
      `已收到，感谢反馈，已转交 旅游群 团队。跟踪链接：${issueUrl}`,
    cmdFeedbackUsage:
      '用法：`/feedback <你的反馈内容>` —— 反馈会直达 旅游群 团队，不会触发 AI 回复。',
    ...
    errorInsufficientCredits:
      '**积分余额不足**\n该机器人所使用的积分不足以覆盖本次模型调用的预估费用。请联系机器人所有者前往 旅游群 网页端充值积分或升级订阅计划，或在 Agent 设置中切换到费用更低的模型。',
    errorInsufficientMemberBudget:
    ...
    errorSubscriptionPlanLimit:
      '**已达到订阅计划限制**\n该机器人所使用的订阅计划无法覆盖本次请求：可能是本周期的订阅积分已用完，也可能是当前计划不包含该模型。请联系机器人所有者前往 旅游群 网页端升级计划或充值，或在 Agent 的 Provider 设置中配置自定义模型 API。',
    errorInvalidProviderAPIKey:
