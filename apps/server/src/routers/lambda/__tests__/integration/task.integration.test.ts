...
describe('Task Router Integration', () => {
  ...
  it.each([undefined, 'schedule', 'heartbeat'] as const)(
    ...
    async (automationMode) => {
      ...
      expect(stored?.totalTopics).toBe(1);
      expect(await new TaskTopicModel(serverDB, userId).findByTaskId(task.data.id)).toEqual([
        expect.objectContaining({
          operationId: 'op_failed',
          status: 'failed',
          topicId: testTopicId,
  ...
  describe('updateStatusCascade', () => {
    ...
    it('persists successful interrupts before surfacing a partial interrupt failure', async () => {
      ...
      mockInterruptTask.mockImplementation(async function ({
        operationId,
      }: {
        operationId: string;
      }) {
  ...
  describe('human assignee (assigneeUserId)', () => {
    ...
    it('should notify the assignee only on an actual assignment to someone else', async () => {
      ...
      expect(mockNotifyTaskAssigned).toHaveBeenCalledWith({
        actorUserId: userId,
        assigneeUserId: otherUserId,
        eventId: expect.stringContaining(task.data.id),
        taskId: task.data.id,
