...
const BudgetField = memo<{
  ...
  const { format, convert, toUsd, symbol } = useMonthlyExchangeRate();
  const { t } = useTranslation('chat');
  const { allowed: canEdit } = usePermission('create_content');
  const setGoalBudget = useGoalStore((s) => s.setGoalBudget);
  const conversionReady = !money || Number.isFinite(convert(1));
  const displayCap = cap === null || !conversionReady ? null : money ? convert(cap) : cap;
  const [draft, setDraft] = useState<number | null>(displayCap);
  const [saving, setSaving] = useState(false);

  // Re-seed from the server whenever it disagrees and the user is not mid-edit.
  const [committed, setCommitted] = useState(displayCap);
  if (committed !== displayCap && !saving) {
    setCommitted(displayCap);
    setDraft(displayCap);
  }

  const commit = async () => {
    if (!canEdit || !conversionReady || draft === displayCap) return;
    ...
    try {
      ...
      await setGoalBudget(
        goalId,
        money ? { maxTotalCost: draft === null ? null : toUsd(draft) } : { maxRounds: draft },
      );
    } catch (error) {
    ...
    <Flexbox gap={2}>
      ...
      <Flexbox horizontal align={'center'} gap={8}>
        ...
        <InputNumber
          ...
          controls={false}
          disabled={!canEdit || saving || !conversionReady}
          min={0}
          placeholder={conversionReady ? t('goalProcess.metricDetail.budget.uncapped') : '—'}
          size={'small'}
          style={{ width: 120 }}
          value={draft}
          variant={'filled'}
