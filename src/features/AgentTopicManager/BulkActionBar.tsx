...
const BulkActionBar = memo(
  ({
    ...
    const storedIds = useTopicsViewStore((s) => s.selectedIds);
    const selectedIds = visibleIds ? storedIds.filter((id) => visibleIds.includes(id)) : storedIds;
    const selectAll = useTopicsViewStore((s) => s.selectAll);
    useEffect(() => {
      if (visibleIds && storedIds.some((id) => !visibleIds.includes(id))) {
        selectAll(storedIds.filter((id) => visibleIds.includes(id)));
      }
    }, [visibleIds, storedIds, selectAll]);
    ...
    const handleBatchDelete = useCallback(() => {
      if (busy || deletingRef.current || !selectedIds.length) return;
      void confirmRemoveTopic({
        content: t('bulkDelete.topicDescription', { ns: 'common', count: selectedIds.length }),
        okText: t('management.bulk.delete'),
        onConfirm: async (removeFiles) => {
          if (!activeRef.current || currentScope.current !== scope || deletingRef.current) return;
          ...
          try {
            ...
            if (failed.length) {
              if (activeRef.current && currentScope.current === scope) selectAll(failed);
              toast.error(t('bulkDelete.partialFailure', { ns: 'common', count: failed.length }));
            } else if (activeRef.current && currentScope.current === scope) exitSelectMode();
          } finally {
    ...
    }, [selectedIds, t, removeTopic, exitSelectMode, selectAll, busy, scope]);

    if (selectedIds.length === 0 && !visibleIds) return null;
