...
const Toolbar = memo<ToolbarProps>(
  ({
    ...
    const handleArchiveStale = useCallback(async () => {
      ...
      try {
        if (getArchiveTopics) candidates = await getArchiveTopics();
      } catch {
        toast.error(t('operationFailed', { ns: 'common', defaultValue: '操作失败，请重试' }));
        return;
      }
      const cutoff = Date.now() - THREE_MONTHS_MS;
      const stale = candidates.filter((tp) => {
        if (
          tp.status === 'completed' ||
          tp.status === 'archived' ||
          tp.status === 'running' ||
          tp.status === 'waitingForHuman' ||
          tp.status === 'scheduled'
        )
          return false;
        const updated =
    ...
    }, [topics, updateTopicStatus, t, getArchiveTopics]);
