...
const KanbanBoard = memo<KanbanBoardProps>(
  ({ agentId, groupId, options, projectId, routeScope }) => {
    ...
    const { error, isLoading, isQueryScopeCurrent, mutate } = useFetchTaskGroupList(
      groupId
        ? { groupId, automated: false, excludeStatuses, groupBy }
        : projectId
    ...
    const hiddenPanelCollapsed = useGlobalStore(
      systemStatusSelectors.taskKanbanHiddenPanelCollapsed,
    );
    ...
    }, [agentId, groupId, canEditTask, navigate, projectId]);
    ...
    const handleToggleHiddenPanel = useCallback(
      (collapsed: boolean) => {
        updateSystemStatus(
          { taskKanbanHiddenPanelCollapsed: collapsed },
          'toggleKanbanHiddenPanel',
        );
      },
      [updateSystemStatus],
    );
          ...
          {activeTask ? (
            <div
              style={{
                ...
                border: '0.5px solid var(--lobe-color-border-secondary, #f0f0f0)',
                borderRadius: 8,
    ...
    return (
      <AsyncBoundary
        ...
        isLoading={
          isLoading || (!isQueryScopeCurrent && !error) || (!isTaskGroupListInit && !error)
        }
