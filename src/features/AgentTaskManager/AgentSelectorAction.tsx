...
const AgentSelectorAction = memo<AgentSelectorActionProps>(
  ({ onAgentChange, assistantId, assistantTitle, allowedAgentIds, ariaLabel }) => {
    ...
    const builtinTaskAgentId = useAgentStore(builtinAgentSelectors.taskAgentId);
    const taskAgentId = assistantId ?? builtinTaskAgentId;
    ...
    const workspaceAgents = useMemo<SidebarAgentItem[]>(() => {
      ...
      const merged = [...pinnedAgents, ...groupedItems, ...ungroupedAgents].filter(
        (agent) =>
          agent.type === 'agent' && (!allowedAgentIds || allowedAgentIds.includes(agent.id)),
      );

      if (taskAgentId && (assistantTitle || !merged.some((agent) => agent.id === taskAgentId))) {
        return [
          {
            ...
            id: taskAgentId,
            pinned: false,
            title: assistantTitle ?? t('taskManager.agent', { ns: 'topic' }),
            type: 'agent' as const,
            updatedAt: new Date(),
          },
          ...merged.filter((agent) => agent.id !== taskAgentId),
        ];
      }
    ...
    }, [privateAgentGroups, privatePinnedAgents, privateUngroupedAgents, allowedAgentIds]);
    ...
    return (
      ...
      >
        <Center
          horizontal
          aria-label={ariaLabel ?? assistantTitle ?? t('taskManager.agent', { ns: 'topic' })}
