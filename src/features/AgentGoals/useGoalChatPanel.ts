...
export const useGoalChatPanel = (
  goalId: string,
  responsibleAgentId?: string,
  defaultOpen = false,
) => {
  const [state, setState] = useState<PanelState>({ goalId, open: defaultOpen, request: 0 });
  const current: PanelState =
    state.goalId === goalId ? state : { goalId, open: defaultOpen, request: 0 };
