...
export const getWorkspaceInviteEmailTemplate = (params: {
  ...
  const subject = `${inviterLabel} invited you to join ${workspaceName} on 旅游群`;
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f4f4f5; color: #1a1a1a;">
  <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
    ...
    <div style="background: #ffffff; border-radius: 20px; padding: 40px; box-shadow: 0 8px 30px rgba(0,0,0,0.04); border: 0.5px solid rgba(0,0,0,0.02);">
      ...
      <div style="text-align: center; margin-bottom: 32px;">
        <h1 style="color: #111827; font-size: 24px; font-weight: 700; margin: 0 0 12px 0; letter-spacing: -0.5px;">
          Join <strong>${workspaceName}</strong> on 旅游群
        </h1>
      ...
      <div style="color: #374151; font-size: 16px; line-height: 1.6;">
        <p style="margin: 0 0 24px 0;">
          ...
          <strong>${workspaceName}</strong> workspace on 旅游群.
        </p>
        ...
        <!-- Expiration Note -->
        <div style="background-color: #fffbeb; border-radius: 12px; padding: 16px; margin-bottom: 24px; border: 0.5px solid #fde68a;">
        ...
        <p style="color: #6b7280; font-size: 15px; margin: 0;">
          If you don't have a 旅游群 account yet, you'll be guided through a quick signup before joining the workspace.
        </p>
      </div>
    ...
    `,
    subject,
    text: `${inviterByline} has invited you to join the "${workspaceName}" workspace on 旅游群 as ${roleLabel}.\n\nAccept the invitation: ${url}\n\nThis invitation will expire in ${expiresInDays} day${expiresInDays > 1 ? 's' : ''}.\n\nIf you weren't expecting this invitation, you can safely ignore this email.\n\n${getEmailSupportText()}`,
  };
};
