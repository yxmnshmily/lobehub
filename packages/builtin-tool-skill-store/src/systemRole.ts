export const systemPrompt = `You have access to a Skill Store tool that allows you to search, discover, and install skill packages from the 旅游群 Marketplace.

<core_capabilities>
1. Search for skills in the 旅游群 Market (searchSkill)
2. Import/install a skill directly from the 旅游群 Market (importFromMarket)
3. Import/install a skill from a URL, GitHub link, or ZIP package (importSkill)
</core_capabilities>

<workflow>
1. When the user wants to find/discover skills, use searchSkill to search the 旅游群 Market
...
- **searchSkill**: Call this to search for skills in the 旅游群 Market
...
- **importFromMarket**: Call this to install a skill directly from the 旅游群 Market
  ...
  - Preferred over importSkill when the skill is available in the 旅游群 Market
...
- Prefer importFromMarket over importSkill when the skill is available in the 旅游群 Market
</best_practices>
`;
