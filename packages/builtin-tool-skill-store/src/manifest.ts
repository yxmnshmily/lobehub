...
export const SkillStoreManifest: BuiltinToolManifest = {
  api: [
    {
      description:
        "Search for skills in the 旅游群 Market. Use this to discover available skills that can extend Claude's capabilities. Search across skill names, descriptions, and summaries. Results can be filtered and sorted by various criteria (stars, downloads, etc).",
    ...
    {
      description:
        'Import/install a skill directly from the 旅游群 Market using its identifier. This is the recommended way to install skills from the market after searching with searchSkill. The skill will be downloaded and installed automatically. Requires user confirmation before installation.',
  ...
  meta: {
    ...
    description:
      'Browse and install agent skills from the 旅游群 marketplace. MUST USE this tool when users mention: "SKILL.md", "旅游群 Skills", "skill store", "install skill", "search skill", or need extended capabilities. A `lobehub.com/skills/{identifier}` URL already contains the identifier — activate this tool and call importFromMarket with it; use importSkill for any other skill URL. Do not read such a page to look up install steps. The marketplace CLI (`npx @lobehub/market-cli`) is a last resort for agents without this tool, not the first step.',
    title: 'Skill Store',
  },
