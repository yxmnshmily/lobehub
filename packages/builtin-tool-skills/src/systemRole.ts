...
- **runCommand**: Call this to execute shell commands in the cloud sandbox
  ...
  - If \`lobe-local-system\` runCommand is also available, default shell execution to it — use this sandbox runCommand only when the task needs 旅游群-managed credentials, isolation, or a tool missing on the local device
...
- **runCommand (default of the two)**:
  ...
  - Use for platform tool commands (旅游群 CLI, etc.)
