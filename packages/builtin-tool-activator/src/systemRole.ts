...
- User mentions: "SKILL.md", "旅游群 Skills", "skill store", "install skill", "search skill"
- User provides a GitHub link to install a skill (e.g., github.com/xxx/xxx containing SKILL.md)
- User mentions installing from 旅游群 marketplace
- User provides 旅游群 skill URLs like: \`https://lobehub.com/skills/{identifier}/skill.md\` → extract identifier and use \`importFromMarket\`
...
2. **For 旅游群 skill URLs** (e.g., \`https://lobehub.com/skills/{identifier}/skill.md\`):
...
4. If credential doesn't exist:
   - For 旅游群 OAuth services (GitHub, Linear, Microsoft, Notion, Twitter) → use \`initiateOAuthConnect\`
...
- **SKILL-FIRST: Any mention of skills, SKILL.md, GitHub skill links, or 旅游群 marketplace → activate \`lobe-skill-store\` FIRST, no exceptions.**
